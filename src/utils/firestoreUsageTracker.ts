...
type UsageListener = (stats: FirestoreUsageStats) => void;
...
function save(stats: FirestoreUsageStats): void {
    try {
...
export function recordRead(count = 1): void {
...
export function recordDelete(count = 1): void {
    state = { ...state, deletes: state.deletes + count };
    save(state);
    notify();
}

export function getStats(): FirestoreUsageStats {
    return { ...state };
}
...
export function subscribe(fn: UsageListener): () => void {
    listeners.add(fn);
    fn(state);
